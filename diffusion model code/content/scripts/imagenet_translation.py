...
def copy_imagenet_dataset(val_dir, classes):
    ...
    for source_label in classes:
        ...
        for i, filename in enumerate(filenames):
            ...
            # 添加调试打印语句
            logger.log(f"Attempting to copy file: {path}")

            if not os.path.exists(path):
                logger.log(f"File not found: {path}, skipping.")
                continue
...
def main():
    ...
    # 加载数据：使用 load_source_data_for_domain_translation 函数加载用于翻译的图像数据。
    # 这个函数负责从指定的源类别中加载图像，并将它们转换为模型可以处理的格式。
...
def create_argparser():
    ...
    parser.add_argument(
        ...
        help="The local directory containing ImageNet validation dataset, "
             "containing filenames like ILSVRC2012_val_000XXXXX.JPEG."
    )
